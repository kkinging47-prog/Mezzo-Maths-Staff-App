...
function birthdayDate(dateOfBirth?: string | null) {
...
export function Admin() {
  ...
  const [payrollForm, setPayrollForm] = useState({ staff_id: '', year: currentYear, month: '', basic_salary: '', allowances: '0', deductions: '0', paid_on: '' });
  ...
  const staffOptions = useMemo(() => staff.map((row) => ({ value: row.id, label: staffLabel(row) })), [staff]);
  const filteredStaffOptions = useMemo(() => {
    const query = normalize(staffPickerSearch);
    if (!query) return staffOptions;
    return staffOptions.filter((option) => normalize(option.label).includes(query));
  }, [staffOptions, staffPickerSearch]);
  const selectedBirthdayStaff = staff.find((row) => row.id === birthdayForm.staff_id);
  const selectedPayrollStaff = staff.find((row) => row.id === payrollForm.staff_id);
  const selectedPayrollMonth = payrollForm.year && payrollForm.month ? `${payrollForm.year}-${payrollForm.month}-01` : '';
  const selectedPayrollMonthKey = monthKey(selectedPayrollMonth);
  ...
  async function loadData() {
    const [{ data: profileData, error: profileError }, { data: schoolData, error: schoolError }, { data: requestData, error: requestError }, { data: loanData }, { data: creditData }, { data: attendanceData }] = await Promise.all([
    ...
    setLoanRepayments(loanData || []); setCreditUnionRows(creditData || []); setAttendanceDeductions(attendanceData || []);
  ...
  const payrollPreview = useMemo(() => {
    const basic = Number(payrollForm.basic_salary || 0);
    const allowances = Number(payrollForm.allowances || 0);
    const adminDeduction = Number(payrollForm.deductions || 0);
    const ssnitNumber = String(selectedPayrollStaff?.ssnit_number || '').trim();
    const ssnit = ssnitNumber ? Number((basic * SSNIT_EMPLOYEE_RATE).toFixed(2)) : 0;
    const paye = calculatePayeMonthly(basic);
    const loan = loanRepayments.filter((row) => row.staff_id === payrollForm.staff_id && monthKey(row.repayment_month) === selectedPayrollMonthKey).reduce((sum, row) => sum + Number(row.scheduled_amount || 0), 0);
    const credit = creditUnionRows.filter((row) => row.staff_id === payrollForm.staff_id && monthKey(row.contribution_month) === selectedPayrollMonthKey).reduce((sum, row) => sum + Number(row.amount || 0), 0);
    const attendance = attendanceDeductions.filter((row) => row.staff_id === payrollForm.staff_id && monthKey(row.work_date) === selectedPayrollMonthKey).reduce((sum, row) => sum + Number(row.amount || 0), 0);
    const totalDeductions = adminDeduction + ssnit + paye + loan + credit + attendance;
    const net = basic + allowances - totalDeductions;
    return { basic, allowances, adminDeduction, ssnitNumber, ssnit, paye, loan, credit, attendance, totalDeductions, net };
  }, [payrollForm, selectedPayrollStaff?.ssnit_number, loanRepayments, creditUnionRows, attendanceDeductions, selectedPayrollMonthKey]);
  ...
  async function createPayroll(event: FormEvent) {
    ...
    if (!profile) return;
    if (!payrollForm.staff_id || !selectedPayrollMonth || Number(payrollForm.basic_salary || 0) < 0) { fail(new Error('Select staff, month, year and enter valid salary details before approval.')); return; }
    const { error } = await supabase.from('payrolls').insert({ staff_id: payrollForm.staff_id, month: selectedPayrollMonth, basic_salary: Number(payrollForm.basic_salary || 0), allowances: Number(payrollForm.allowances || 0), deductions: Number(payrollForm.deductions || 0), paid_on: payrollForm.paid_on || null, status: 'approved', approved_by: profile.id, approved_at: new Date().toISOString() });
    if (error) fail(error);
    else {
      await notifyStaff(payrollForm.staff_id, 'Payslip approved', 'Your monthly payslip has been approved and is now available under Letters & Payslips.');
      ok('Payslip approved and published to the staff page.');
      setPayrollForm((prev) => ({ ...prev, month: '', basic_salary: '', allowances: '0', deductions: '0', paid_on: '' }));
    }
  }
  ...
  async function createBirthdayPost(event: FormEvent) {
    ...
    try {
      const dataUrl = birthdayPreview || await generateBirthdayCardImage({ staffName: birthdayForm.display_name, position: birthdayForm.position, birthdayDate: birthdayForm.birthday_date, message: birthdayForm.message, photoUrl: selectedBirthdayStaff?.photo_url });
      ...
      const { error: postError } = await supabase.from('company_posts').insert({ title: `Happy Birthday ${birthdayForm.display_name}`, body: birthdayForm.message, priority: 'important', post_type: 'birthday', image_url: data.publicUrl, image_path: path, author_id: profile.id });
      if (postError) throw postError;
      setBirthdayPreview(data.publicUrl);
      ok('Birthday e-card posted to the dashboard.');
    } catch (error: any) { fail(error); } finally { setBirthdayBusy(false); }
  }
  ...
  return <section>
    ...
    <div className="grid two">
      ...
      <form className="panel form-grid payroll-panel" onSubmit={createPayroll}>
        <h2>Create Monthly Payslip Data</h2>
        <div className="payroll-form-grid">
          <label className="wide-field">Search Staff<input value={staffPickerSearch} onChange={(e) => setStaffPickerSearch(e.target.value)} placeholder="Type name, email, staff number or position" /></label>
          <label className="wide-field">Staff<select value={payrollForm.staff_id} onChange={(e) => setPayrollForm({ ...payrollForm, staff_id: e.target.value })}>{filteredStaffOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
          <label>Year<input type="number" min="2020" max="2100" value={payrollForm.year} onChange={(e) => setPayrollForm({ ...payrollForm, year: e.target.value })} required /></label>
          <label>Month<select value={payrollForm.month} onChange={(e) => setPayrollForm({ ...payrollForm, month: e.target.value })} required><option value="">Select month</option>{months.map((month) => <option key={month.value} value={month.value}>{month.label}</option>)}</select></label>
          <label>Basic Salary<input type="number" value={payrollForm.basic_salary} onChange={(e) => setPayrollForm({ ...payrollForm, basic_salary: e.target.value })} required /></label>
          <label>Allowances<input type="number" value={payrollForm.allowances} onChange={(e) => setPayrollForm({ ...payrollForm, allowances: e.target.value })} /></label>
          <label className="wide-field">Other / Admin Deductions<input type="number" value={payrollForm.deductions} onChange={(e) => setPayrollForm({ ...payrollForm, deductions: e.target.value })} /></label>
          <label className="wide-field">Paid On<input type="date" value={payrollForm.paid_on} onChange={(e) => setPayrollForm({ ...payrollForm, paid_on: e.target.value })} /></label>
        </div>
        <div className="approval-card approved payroll-preview-card"><strong>Final Payslip Preview</strong><span>Staff: {selectedPayrollStaff?.full_name || selectedPayrollStaff?.email || '-'}</span><span>Month: {payrollForm.month ? `${months.find((month) => month.value === payrollForm.month)?.label} ${payrollForm.year}` : '-'}</span><span>Basic salary: {money(payrollPreview.basic)}</span><span>Allowances: {money(payrollPreview.allowances)}</span>{payrollPreview.ssnit > 0 && <span>SSNIT Employee Deduction: {money(payrollPreview.ssnit)}</span>}<span>PAYE Tax: {money(payrollPreview.paye)}</span>{payrollPreview.adminDeduction > 0 && <span>Other / Admin Deductions: {money(payrollPreview.adminDeduction)}</span>}{payrollPreview.loan > 0 && <span>Staff Loan Repayment: {money(payrollPreview.loan)}</span>}{payrollPreview.credit > 0 && <span>Credit Union Contribution: {money(payrollPreview.credit)}</span>}{payrollPreview.attendance > 0 && <span>Attendance Deductions: {money(payrollPreview.attendance)}</span>}<span>Total deductions: {money(payrollPreview.totalDeductions)}</span><span>Net pay: {money(payrollPreview.net)}</span></div>
        <button className="primary">Approve & Publish Payslip</button>
      </form>
