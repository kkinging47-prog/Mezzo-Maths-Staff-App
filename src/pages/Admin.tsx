...
export function Admin() {
  ...
  const payrollPreview = useMemo(() => {
    const selectedMonth = monthKey(payrollForm.month);
    ...
    const loan = loanRepayments.filter((row) => row.staff_id === payrollForm.staff_id && monthKey(row.repayment_month) === selectedMonth).reduce((sum, row) => sum + Number(row.scheduled_amount || 0), 0);
    const credit = creditUnionRows.filter((row) => row.staff_id === payrollForm.staff_id && monthKey(row.contribution_month) === selectedMonth).reduce((sum, row) => sum + Number(row.amount || 0), 0);
    const attendance = attendanceDeductions.filter((row) => row.staff_id === payrollForm.staff_id && monthKey(row.work_date) === selectedMonth).reduce((sum, row) => sum + Number(row.amount || 0), 0);
  ...
  }, [payrollForm, selectedPayrollStaff?.ssnit_number, loanRepayments, creditUnionRows, attendanceDeductions]);
  ...
  async function createPayroll(event: FormEvent) { event.preventDefault(); if (!profile) return; if (!payrollForm.staff_id || !payrollForm.month || Number(payrollForm.basic_salary || 0) < 0) { fail(new Error('Select staff, month and enter valid salary details before approval.')); return; } const { error } = await supabase.from('payrolls').insert({ staff_id: payrollForm.staff_id, month: payrollForm.month, basic_salary: Number(payrollForm.basic_salary || 0), allowances: Number(payrollForm.allowances || 0), deductions: Number(payrollForm.deductions || 0), paid_on: payrollForm.paid_on || null, status: 'approved', approved_by: profile.id, approved_at: new Date().toISOString() }); if (error) fail(error); else { await notifyStaff(payrollForm.staff_id, 'Payslip approved', 'Your monthly payslip has been approved and is now available under Letters & Payslips.'); ok('Payslip approved and published to the staff page.'); setPayrollForm((prev) => ({ ...prev, month: '', basic_salary: '', allowances: '0', deductions: '0', paid_on: '' })); } }
  ...
  return <section>
    ...
    <div className="grid two">
      ...
      <form className="panel form-grid" onSubmit={assignSchool}><h2>Assign Staff to School</h2><label>Search Staff<input value={staffPickerSearch} onChange={(e) => setStaffPickerSearch(e.target.value)} placeholder="Type name, email, staff number or position" /></label><label>Staff<select value={selectedStaffId} onChange={(e) => setSelectedStaffId(e.target.value)}>{filteredStaffOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label>School<select value={selectedSchoolId} onChange={(e) => setSelectedSchoolId(e.target.value)}>{schools.map((school) => <option key={school.id} value={school.id}>{school.name}</option>)}</select></label><button className="primary">Assign School</button></form>
      ...
      <form className="panel form-grid" onSubmit={createPayroll}><h2>Create & Approve Monthly Payslip</h2><p className="hint">Check the final payslip figures below before clicking Approve & Publish. Staff will only see approved payslips.</p><label>Search Staff<input value={staffPickerSearch} onChange={(e) => setStaffPickerSearch(e.target.value)} placeholder="Type name, email, staff number or position" /></label><label>Staff<select value={payrollForm.staff_id} onChange={(e) => setPayrollForm({ ...payrollForm, staff_id: e.target.value })}>{filteredStaffOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label>Month<input type="date" value={payrollForm.month} onChange={(e) => setPayrollForm({ ...payrollForm, month: e.target.value })} required /></label><div className="grid two"><label>Basic Salary<input type="number" value={payrollForm.basic_salary} onChange={(e) => setPayrollForm({ ...payrollForm, basic_salary: e.target.value })} required /></label><label>Allowances<input type="number" value={payrollForm.allowances} onChange={(e) => setPayrollForm({ ...payrollForm, allowances: e.target.value })} /></label></div><label>Other / Admin Deductions<input type="number" value={payrollForm.deductions} onChange={(e) => setPayrollForm({ ...payrollForm, deductions: e.target.value })} /></label><label>Paid On<input type="date" value={payrollForm.paid_on} onChange={(e) => setPayrollForm({ ...payrollForm, paid_on: e.target.value })} /></label><div className="approval-card approved"><strong>Final Payslip Preview</strong><span>Staff: {selectedPayrollStaff?.full_name || selectedPayrollStaff?.email || '-'}</span><span>Basic Salary: {money(payrollPreview.basic)}</span><span>Allowances: {money(payrollPreview.allowances)}</span>{payrollPreview.ssnit > 0 && <span>SSNIT Deduction ({payrollPreview.ssnitNumber}): {money(payrollPreview.ssnit)}</span>}<span>PAYE Tax: {money(payrollPreview.paye)}</span>{payrollPreview.adminDeduction > 0 && <span>Other / Admin Deductions: {money(payrollPreview.adminDeduction)}</span>}{payrollPreview.loan > 0 && <span>Staff Loan Repayment: {money(payrollPreview.loan)}</span>}{payrollPreview.credit > 0 && <span>Credit Union Contribution: {money(payrollPreview.credit)}</span>}{payrollPreview.attendance > 0 && <span>Attendance Deductions: {money(payrollPreview.attendance)}</span>}<span>Total Deductions: {money(payrollPreview.totalDeductions)}</span><span>Net Pay: {money(payrollPreview.net)}</span></div><button className="primary">Approve & Publish Payslip</button></form>
      <form className="panel form-grid birthday-admin" onSubmit={createBirthdayPost}><h2>Generate Birthday E-Card</h2><label>Search Staff<input value={staffPickerSearch} onChange={(e) => setStaffPickerSearch(e.target.value)} placeholder="Type name, email, staff number or position" /></label><label>Staff<select value={birthdayForm.staff_id} onChange={(e) => setBirthdayForm({ ...birthdayForm, staff_id: e.target.value })}>{filteredStaffOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><div className="grid two"><label>Name on Card<input value={birthdayForm.display_name} onChange={(e) => setBirthdayForm({ ...birthdayForm, display_name: e.target.value })} required /></label><label>Position / Role<input value={birthdayForm.position} onChange={(e) => setBirthdayForm({ ...birthdayForm, position: e.target.value })} /></label></div><label>Birthday Date<input type="date" value={birthdayForm.birthday_date} onChange={(e) => setBirthdayForm({ ...birthdayForm, birthday_date: e.target.value })} /></label>{!selectedBirthdayStaff?.photo_url && <p className="warning">This staff member has no profile photo yet. Ask the staff to upload one from My Details.</p>}<label>Birthday Message<textarea value={birthdayForm.message} onChange={(e) => setBirthdayForm({ ...birthdayForm, message: e.target.value })} required /></label><div className="button-row"><button className="primary" type="button" disabled={birthdayBusy} onClick={generateBirthdayPreview}>{birthdayBusy ? 'Working...' : 'Preview Card'}</button><button className="primary" disabled={birthdayBusy}>{birthdayBusy ? 'Posting...' : 'Post to Dashboard'}</button></div>{birthdayPreview && <div className="birthday-preview-card"><img src={birthdayPreview} alt="Generated birthday e-card preview" /><a className="download-link" href={birthdayPreview} download={`${safeName(birthdayForm.display_name)}-birthday-card.jpg`}>Download JPG</a></div>}</form>
    </div>
  </section>;
}
