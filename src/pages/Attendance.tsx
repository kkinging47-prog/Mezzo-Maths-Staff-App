...
export function Attendance() {
  ...
  async function loadData() {
    ...
    const schoolList = supervisor ? (data || []) as School[] : (data || []).map((row: any) => row.schools).filter(Boolean) as School[];
  ...
  useEffect(() => { loadData(); }, [profile?.id, profile?.position, admin]);
  ...
  async function handleCheckIn() {
    if (!profile || !selectedSchool || admin) return;
  ...
  async function handleCheckOut(auto = false) {
    if (!profile || !openRecord || admin) return;
    setBusy(true); setMessage('');
    try {
      const update: Record<string, any> = { check_out_at: new Date().toISOString(), status: auto ? 'auto_checked_out' : 'checked_out' };
      if (!auto && openRecord.schools) {
        const position = await getCurrentPosition();
        update.check_out_lat = position.latitude;
        update.check_out_lng = position.longitude;
        update.check_out_distance_m = schoolHasGps(openRecord.schools) ? distanceInMeters(position.latitude, position.longitude, Number(openRecord.schools.latitude), Number(openRecord.schools.longitude)) : 0;
      }
      const { error } = await supabase.from('attendance').update(update).eq('id', openRecord.id).eq('staff_id', profile.id);
      if (error) throw error;
      setType('success'); setMessage(auto ? 'You have been automatically checked out.' : 'Checked out successfully.'); await loadData();
    } catch (error: any) { setType('error'); setMessage(error.message || 'Check-out failed.'); }
    finally { setBusy(false); }
  }

  if (admin) return <section><div className="page-header"><div><h1>Attendance</h1><p>Admin accounts are exempted from attendance check-in.</p></div></div><div className="panel"><h2>Admin Exemption</h2><p className="status info">You are signed in as an admin. Admin accounts are not required to check in and cannot receive attendance deductions.</p></div></section>;

  return <section><div className="page-header"><div><h1>Attendance</h1><p>Check in only within the approved school radius. If the selected school has no GPS saved yet, your first check-in will save its GPS location.</p></div></div><StatusMessage message={message} type={type} /><div className="grid two"><div className="panel"><h2>Today</h2>{openRecord ? <div><p>You are currently checked in at <strong>{openRecord.schools?.name || 'selected school'}</strong>.</p><p className="muted">Check-in time: {new Date(openRecord.check_in_at).toLocaleString()}</p><button className="danger" disabled={busy} onClick={() => handleCheckOut(false)}>{busy ? 'Working...' : 'Check out now'}</button></div> : <div className="form-grid"><label>School<select value={schoolId} onChange={(e) => setSchoolId(e.target.value)}>{schools.map((school) => <option key={school.id} value={school.id}>{school.name}</option>)}</select></label>{schools.length === 0 && <p className="warning">No school has been assigned to your account yet. Contact admin.</p>}{selectedSchool && !schoolHasGps(selectedSchool) && <p className="status info">This school has no saved GPS yet. Your check-in location will be used to set it.</p>}<CameraCapture onCapture={(file) => setSelfie(file)} /><button className="primary" disabled={busy || schools.length === 0} onClick={handleCheckIn}>{busy ? 'Checking...' : 'Check in with location + photo'}</button></div>}</div><div className="panel"><h2>Geofence Rule</h2><p>Default radius is <strong>{selectedSchool?.radius_m || 100} meters</strong>. The app compares your live GPS position with the selected school’s saved coordinates.</p><p className="muted">For schools without saved GPS, the first approved check-in stores the current location as the school location.</p></div></div><h2>Recent Attendance</h2><div className="table-card"><table><thead><tr><th>Date</th><th>School</th><th>Check in</th><th>Check out</th><th>Status</th></tr></thead><tbody>{records.map((row) => <tr key={row.id}><td>{row.work_date}</td><td>{row.schools?.name || '-'}</td><td>{row.check_in_at ? new Date(row.check_in_at).toLocaleTimeString() : '-'}</td><td>{row.check_out_at ? new Date(row.check_out_at).toLocaleTimeString() : '-'}</td><td><span className="pill">{row.status}</span></td></tr>)}</tbody></table></div></section>;
}
