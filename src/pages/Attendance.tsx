...
export function Attendance() {
  ...
  async function loadData() {
    if (!profile) return;
    const { data: assigned, error } = await supabase.from('staff_school_assignments').select('schools(*)').eq('staff_id', profile.id);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => handleCheckOut(true), target.getTime() - Date.now());
  ...
  async function handleCheckIn() {
    ...
    try {
      ...
      if (error) throw error;
      setType('success'); setMessage(`Checked in successfully at ${selectedSchool.name}.`);
      await loadData();
  ...
  async function handleCheckOut(auto = false) {
    ...
    try {
      ...
      if (error) throw error;
      setType('success'); setMessage(auto ? 'You have been automatically checked out.' : 'Checked out successfully.');
      await loadData();
