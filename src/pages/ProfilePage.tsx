...
export function ProfilePage() {
  ...
  async function updatePassword(event: FormEvent) {
    event.preventDefault();
    setMessage('');
    if (passwordForm.newPassword.length < 6) { setType('error'); setMessage('New password must be at least 6 characters.'); return; }
    if (passwordForm.newPassword !== passwordForm.confirmPassword) { setType('error'); setMessage('The two passwords do not match.'); return; }
  ...
  return <section>
    <div className="page-header"><div><h1>My Staff Details</h1><p>Keep your employment, emergency details, staff photo, email and password accurate.</p></div></div>
    <StatusMessage message={message} type={type} />
    <form className="panel form-grid" onSubmit={submit}>
      <div className="profile-photo-row"><div className="profile-photo-preview">{photoPreview ? <img src={photoPreview} alt="Staff profile preview" /> : <span>No photo</span>}</div><div><label>Upload / Update Profile Image<input type="file" accept="image/*" onChange={selectPhoto} /></label><p className="hint">Images are reduced before upload and used for profile and birthday e-cards.</p></div></div>
