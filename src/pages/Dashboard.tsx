...
export function Dashboard() {
  ...
  async function loadSalaryDate() { const { data } = await supabase.from('company_settings').select('value').eq('key', 'salary_pay_date').maybeSingle(); setSalaryDate(data?.value || ''); }
  ...
  return <section>
    ...
    <div className="grid two"><QuickAttendance /><div className="panel dashboard-profile-card">{profile?.photo_url ? <img className="staff-avatar" src={profile.photo_url} alt="Staff profile" /> : <div className="staff-avatar placeholder">{initials(profile?.full_name, profile?.email)}</div>}<div><h2>Welcome{profile?.full_name ? `, ${profile.full_name}` : ''}</h2><p className="muted">{profile?.position || 'Staff'} · {profile?.department || 'Department not set'}</p><p><strong>Staff No:</strong> {profile?.staff_no || 'Not set'}<br /><strong>Salary Pay Date:</strong> {salaryDate ? new Date(salaryDate).toLocaleDateString() : 'Not announced'}</p></div></div></div>
