...
export function Dashboard() {
  ...
  async function loadPosts() {
    const { data, error } = await supabase.from('company_posts').select('*, profiles(full_name,email)').is('archived_at', null).order('created_at', { ascending: false }).limit(50);
    if (error) setMessage(error.message); else setPosts((data || []) as CompanyPost[]);
  }
  async function loadComments(postIds: string[]) {
    if (!postIds.length) return;
    const { data } = await supabase.from('post_comments').select('*, profiles(full_name,email)').in('post_id', postIds).order('created_at', { ascending: true });
    const grouped: Record<string, Comment[]> = {};
    (data || []).forEach((comment: any) => { grouped[comment.post_id] ||= []; grouped[comment.post_id].push(comment as Comment); });
    setComments(grouped);
  }
  ...
  async function loadSalaryDate() {
    const { data } = await supabase.from('company_settings').select('value').eq('key', 'salary_pay_date').maybeSingle();
    setSalaryDate(data?.value || '');
  }

  useEffect(() => { loadPosts(); loadScoreboard(); loadSalaryDate(); }, []);
  ...
  async function submitComment(event: FormEvent, postId: string) {
    event.preventDefault(); const body = newComment[postId]?.trim(); if (!body || !profile) return;
    const { error } = await supabase.from('post_comments').insert({ post_id: postId, body, author_id: profile.id });
    if (error) setMessage(error.message); else { setNewComment((prev) => ({ ...prev, [postId]: '' })); await loadComments(posts.map((post) => post.id)); }
  }
  ...
  return <section>
    ...
    <div className="grid two"><QuickAttendance /><div className="panel stat-panel"><span>Role</span><strong>{profile?.role || 'staff'}</strong><span>Staff No.</span><strong>{profile?.staff_no || 'Not set'}</strong><span>Salary Pay Date</span><strong>{salaryDate ? new Date(salaryDate).toLocaleDateString() : 'Not announced'}</strong></div></div>
    <div className="panel"><h2>Weekly Attendance Scores</h2><p className="hint">Scores rank the most consistent staff for the current week. Present day = 10 points; absent day = -3 points.</p><div className="table-card compact-table"><table><thead><tr><th>Rank</th><th>Staff</th><th>Present Days</th><th>Absent</th><th>Score</th></tr></thead><tbody>{scoreboard.map((row, index) => <tr key={row.staff_id}><td>{index + 1}</td><td>{row.name}</td><td>{row.present}</td><td>{row.absent}</td><td><strong>{row.score}</strong></td></tr>)}</tbody></table></div></div>
