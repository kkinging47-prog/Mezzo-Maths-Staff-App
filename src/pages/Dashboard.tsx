...
import { FloatingTeacherPost } from '../components/FloatingTeacherPost';
...
export function Dashboard() {
  ...
  async function loadPosts() {
    const { data, error } = await supabase.from('company_posts').select('*, profiles(full_name,email)').is('archived_at', null).order('created_at', { ascending: false }).limit(30);
  ...
  async function loadSettings() {
    const { data } = await supabase.from('company_settings').select('key,value').in('key', ['salary_pay_date', 'current_academic_year', 'current_term']);
  ...
  async function submitComment(event: FormEvent, postId: string) {
    ...
    if (error) setMessage(error.message); else { setNewComment((prev) => ({ ...prev, [postId]: '' })); await loadPosts(); }
  }
  ...
  return <section>
    ...
    <div className="feed dashboard-updates-first">
      ...
      {posts.map((post) => <article id={`post-${post.id}`} key={post.id} className={`post priority-${post.priority}`}><div className="post-head"><div><strong>{post.title}</strong><span>{new Date(post.created_at).toLocaleString()}</span></div><em>{post.priority}</em></div><p>{post.body}</p>{post.image_url && <div className="post-image-card"><img src={post.image_url} alt={post.title} /><a className="download-link" href={post.image_url} download={`${post.title.replace(/[^a-z0-9]+/gi, '-')}.jpg`}>Download JPG</a></div>}<PostShareButtons post={post} /><div className="comments">{(comments[post.id] || []).map((comment) => <div key={comment.id} className="comment"><strong>{comment.profiles?.full_name || 'Staff'}:</strong> {comment.body}</div>)}<form className="comment-form" onSubmit={(event) => submitComment(event, post.id)}><input placeholder="Write a comment..." value={newComment[post.id] || ''} onChange={(e) => setNewComment((prev) => ({ ...prev, [post.id]: e.target.value }))} /><button>Comment</button></form></div></article>)}
    </div>
