...
export function Dashboard() {
  ...
  async function loadPosts() { const { data, error } = await supabase.from('company_posts').select('*, profiles(full_name,email)').is('archived_at', null).order('created_at', { ascending: false }).limit(30); if (error) setMessage(error.message); else setPosts((data || []) as CompanyPost[]); }
  async function loadComments(postIds: string[]) { if (!postIds.length) { setComments({}); return; } const { data } = await supabase.from('post_comments').select('*, profiles(full_name,email)').in('post_id', postIds).order('created_at', { ascending: true }); const grouped: Record<string, Comment[]> = {}; (data || []).forEach((comment: any) => { grouped[comment.post_id] ||= []; grouped[comment.post_id].push(comment as Comment); }); setComments(grouped); }
  ...
  useEffect(() => { const channel = supabase.channel('company-dashboard-feed').on('postgres_changes', { event: '*', schema: 'public', table: 'company_posts' }, loadPosts).on('postgres_changes', { event: '*', schema: 'public', table: 'post_comments' }, loadPosts).subscribe(); return () => { supabase.removeChannel(channel); }; }, []);
  async function submitComment(event: FormEvent, postId: string) { event.preventDefault(); const body = newComment[postId]?.trim(); if (!body || !profile) return; const { error } = await supabase.from('post_comments').insert({ post_id: postId, body, author_id: profile.id }); if (error) setMessage(error.message); else { setNewComment((prev) => ({ ...prev, [postId]: '' })); await loadPosts(); } }
  return <section>
