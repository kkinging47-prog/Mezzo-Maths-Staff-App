...
export function WorkbookOrders() {
  ...
  const supervisor = useMemo(() => isSupervisor(profile?.position), [profile?.position]);
  ...
  async function load() {
    ...
    const schoolQuery = profile.role === 'admin'
    ...
    if (profile.role !== 'admin' && schoolIds.length === 0) { setOrders([]); setSupplies([]); setCounts([]); return; }
    let orderQuery = supabase.from('workbook_orders').select('*, schools(name), profiles(full_name,email)').eq('academic_year', currentYear).eq('term', currentTerm).order('created_at', { ascending: false });
    let supplyQuery = supabase.from('workbook_supplies').select('*, schools(name), profiles(full_name,email)').eq('academic_year', currentYear).eq('term', currentTerm).order('supplied_on', { ascending: false });
    let countQuery = supabase.from('class_student_counts').select('*, schools(name), profiles(full_name,email)').eq('academic_year', currentYear).eq('term', currentTerm).order('updated_at', { ascending: false });
    if (profile.role !== 'admin') {
      orderQuery = orderQuery.in('school_id', schoolIds);
      supplyQuery = supplyQuery.in('school_id', schoolIds);
      countQuery = countQuery.in('school_id', schoolIds).eq('staff_id', profile.id);
    }
    const [{ data: orderData }, { data: supplyData }, { data: countData }] = await Promise.all([orderQuery, supplyQuery, countQuery]);
    setOrders(orderData || []); setSupplies(supplyData || []); setCounts(countData || []);
  }
  useEffect(() => { load(); }, [profile?.id, profile?.role, profile?.position]);

  async function submitOrder(event: FormEvent) { event.preventDefault(); if (!profile || supervisor) return; const { error } = await supabase.from('workbook_orders').insert({ staff_id: profile.id, school_id: order.school_id, class_name: order.class_name, quantity: Number(order.quantity), academic_year: year, term }); if (error) setMessage(error.message); else { setMessage('Workbook order submitted.'); setOrder((p) => ({ ...p, quantity: '' })); load(); } }
  async function submitCount(event: FormEvent) { event.preventDefault(); if (!profile || supervisor) return; const { error } = await supabase.from('class_student_counts').upsert({ staff_id: profile.id, school_id: countForm.school_id, class_name: countForm.class_name, student_count: Number(countForm.student_count), academic_year: year, term, updated_at: new Date().toISOString() }, { onConflict: 'staff_id,school_id,class_name,academic_year,term' }); if (error) setMessage(error.message); else { setMessage('Student count updated.'); setCountForm((p) => ({ ...p, student_count: '' })); load(); } }
  async function submitSupply(event: FormEvent) { event.preventDefault(); if (!profile || profile.role !== 'admin') return; const { error } = await supabase.from('workbook_supplies').insert({ posted_by: profile.id, school_id: supply.school_id, class_name: supply.class_name, quantity: Number(supply.quantity), supplied_on: supply.supplied_on || new Date().toISOString().slice(0, 10), academic_year: year, term }); if (error) setMessage(error.message); else { setMessage('Supply record posted.'); setSupply((p) => ({ ...p, quantity: '', supplied_on: '' })); load(); } }
