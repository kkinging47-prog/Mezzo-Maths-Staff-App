...
export function Documents() {
  ...
  async function load() {
    ...
    const [{ data: payrollData, error: payrollError }, { data: requestData, error: requestError }, { data: uploadData }, { data: agreementData }, { data: loanData }, { data: creditData }, { data: attendanceData }] = await Promise.all([
      supabase.from('payrolls').select('*').eq('staff_id', profile.id).or('status.eq.approved,status.is.null').order('month', { ascending: false }),
  ...
  const payslipDeductionLines = useMemo<PayslipDeductionLine[]>(() => {
    ...
    if (ssnitDeduction > 0) lines.push({ label: `SSNIT Employee Deduction (${ssnitNumber})`, amount: ssnitDeduction });
    lines.push({ label: 'PAYE Tax', amount: paye });
    if (loanDue > 0) lines.push({ label: 'Staff Loan Repayment', amount: loanDue });
    if (creditUnion > 0) lines.push({ label: 'Credit Union Contribution', amount: creditUnion });
    if (attendance > 0) lines.push({ label: 'Attendance Deductions', amount: attendance });
    return lines;
